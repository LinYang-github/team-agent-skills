<# 用途：批量安装团队 Skill 到 Grok；当前脚本只处理 Grok。
   普通安装：powershell -File .\scripts\install-grok-windows.ps1
   强制安装：powershell -File .\scripts\install-grok-windows.ps1 -Force
#>
param(
    [switch]$Force
)
...
$TargetDir = if ($env:GROK_SKILLS_DIR) { $env:GROK_SKILLS_DIR } else { Join-Path $HOME ".grok\skills" }
...
Write-Host ""
Write-Host "Grok Skill installation complete."
