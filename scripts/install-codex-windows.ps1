<# 用途：批量安装团队 Skill 到 Codex；当前脚本只处理 Codex。
   普通安装：powershell -File .\scripts\install-codex-windows.ps1
   强制安装：powershell -File .\scripts\install-codex-windows.ps1 -Force
#>
param(
    [switch]$Force
)
...
$TargetDir = if ($env:CODEX_SKILLS_DIR) { $env:CODEX_SKILLS_DIR } else { Join-Path $HOME ".codex\skills" }
...
Write-Host ""
Write-Host "Codex Skill installation complete."
