...
- Leave out each skill's root `evals/` directory (evaluation prompts, expected answers, grading material). An installed model can read every file in the plugin, so shipping expected answers invalidates evaluations and exposes them at runtime. Keep evals in the product repository and run them from there; a directory with the same name deeper inside a skill is ordinary content only when the skill actually uses it at runtime.
