...
任务涉及 Go 服务或 Worker 时，必须确认以下结果：

- **配置可覆盖**：遵循通用规范的“命令行 > 环境变量 > 用户配置 > 内部默认值”；监听地址、端口等非敏感用户配置提供 `flag` 或等价命令行入口，启动时完成类型和范围校验。
- **服务可停止**：使用可取消的根 `context` 接收系统信号；HTTP 服务显式配置超时，通过 `Shutdown` 有界停止，后台任务同步取消并等待。
- **并发有容量**：不得按请求无限创建 goroutine；使用有并发上限和队列容量的 Worker、信号量或等价机制，并定义队满时的背压、拒绝或超时行为。
- **失败可观察**：goroutine 的错误回到拥有者，不只写日志；启动失败、运行期关键错误和关闭超时返回非零退出状态。
- **验证含竞态**：至少执行 `gofmt`、`go vet`、相关 `go test`；涉及并发时执行 `go test -race`，涉及关闭时覆盖取消、超时、队满和退出竞争。
...
涉及服务或 Worker 时，按“配置覆盖、停止、容量、失败、验证”报告门禁结果；没有实际执行的命令必须标明未执行。
